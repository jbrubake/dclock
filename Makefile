...
# ----------------------------------------------------------------------
# Makefile generated from "Imake.tmpl" and </tmp/IIf.NNK5fN>
...
# operating system:  Linux 2.6.9 i686 [ELF] (2.6.9)
# libc:	(6.3.3)
# binutils:	(215)
...
# $XdotOrg: xc/config/cf/xorg.cf,v 1.27.2.2 2004/09/17 15:38:18 kem Exp $
...
# from  top Makefile
  BOOTSTRAPCFLAGS = -O2  -pipe -m32 -march=i386 -mtune=pentium4 -pipe

               CC = gcc -m32
               AS = gcc -m32 -c -x assembler -Wa,--noexecstack
...
.SUFFIXES: .cc
    ...
    CXXDEBUGFLAGS = -O2  -pipe -m32 -march=i386 -mtune=pentium4 -fno-strict-aliasing -pipe
CXXDEPENDINCLUDES =
 CXXEXTRA_DEFINES =
CXXEXTRA_INCLUDES =
   ...
   COMPRESSMANCMD = gzip -n
        ...
        MANMACROS = -man
         ...
         COLFLAGS = -b
           ...
           MODCPP = cpp
        MODCFLAGS = $(CFLAGS)
            MODAS = gcc -m32 -c -x assembler -Wa,--noexecstack
       MODASFLAGS =
  ...
  EXTRA_LIBRARIES =
   ...
   PARALLELMFLAGS =
    ...
    INSTKMEMFLAGS = -m 4711
      ...
      CDEBUGFLAGS = -O2  -pipe -m32 -march=i386 -mtune=pentium4 -fno-strict-aliasing -pipe
        CCOPTIONS =
...
# $XdotOrg: xc/config/cf/X11.tmpl,v 1.35 2004/09/03 16:18:18 kem Exp $
...
XTRANS_FAILDEFINES =
    ...
    VENDORMANNAME = X.Org
 VENDORMANVERSION = `echo 6 8 1 | sed -e 's/ /./g' -e 's/^/Version\\\ /'`
...
install:: dclock
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  dclock $(DESTDIR)$(BINDIR)/dclock

all:: dclock.$(MANNEWSUFFIX)

dclock.$(MANNEWSUFFIX): dclock.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname dclock` && \
	   $(LN) `basename dclock.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < dclock.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) dclock.$(MANNEWSUFFIX)

install.man:: dclock.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) dclock.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/dclock.$(MANSUFFIX)
